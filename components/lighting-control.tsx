...
export default function LightingControl({ systemStatus, setSystemStatus, setAlerts }: LightingControlProps) {
  ...
  const [lightingZones, setLightingZones] = useState([
    { id: "device1", name: "Living Room", intensity: 75, status: "on" },
    { id: "device2", name: "Kitchen", intensity: 85, status: "on" },
    { id: "device3", name: "Bedroom", intensity: 45, status: "on" },
    { id: "device4", name: "Bathroom", intensity: 90, status: "on" },
  ])
    ...
    if (systemStatus.lightingAuto && !manualOverride) {
      ...
      if (newIntensity !== systemStatus.lightIntensity) {
        ...
        setLightingZones((prev) =>
          prev.map((zone) => {
            sendLightingCommand(zone.id, newIntensity, true)
            return { ...zone, intensity: newIntensity }
          })
        )
      }
    }
  ...
  const toggleAutoMode = () => {
    ...
    setLightingZones((prev) =>
      prev.map((zone) => {
        sendLightingCommand(zone.id, systemStatus.lightIntensity, newAuto)
        return { ...zone, intensity: systemStatus.lightIntensity }
      })
    )
  ...
  const handleMasterIntensityChange = (value: number[]) => {
    ...
    setLightingZones((prev) =>
      prev.map((zone) => {
        sendLightingCommand(zone.id, newIntensity, false)
        return { ...zone, intensity: newIntensity }
      })
    )
  ...
  const updateZoneIntensity = (zoneId: string, intensity: number) => {
    setLightingZones((prev) =>
      prev.map((zone) => {
        if (zone.id === zoneId) {
          sendLightingCommand(zoneId, intensity, false)
